...
$foo = new Upload($_FILES[$rows["url"]], 'tr_TR'); 
if ($foo->uploaded) {
   $foo->Process($_SERVER['DOCUMENT_ROOT']."/upload/");
   if ($foo->processed) {
	   
	$ad='sonuc'.rand(0,9999).'-'.rand(0,9999);
	$foo->file_new_name_body = $ad;
   $foo->Process($_SERVER['DOCUMENT_ROOT']."/upload/");
   if ($foo->processed) {
	 $vt_row[] = "?";
	 $vt_value[] = $foo->file_dst_name_body.".".$foo->file_dst_name_ext;
   } else {
     $vt_row[] = "?";
	 $vt_value[] = ""; //$foo->error;
   }   
...
if ($foo->uploaded) {
   ...
   if ($foo->processed) {
	 ...
	 $db->veriGuncelle($vt,array($rows["url"]),array($foo->file_dst_name_body.".".$foo->file_dst_name_ext,$id),"id");
   } else {
...
<?php } elseif($action == "edit") { ?>
												<div class="col-lg-12">
													<div class="card-box">
														...
														<p class="text-muted m-b-30 font-13">
									* Zorunlu alanlar <br>
									<b>** Lütfen Docx veya Pdf Dosyaları yükleyiniz.</b>
								</p>
								<?php
									if($verot_alert == 2) { alert_error("Yüklenirken bir hata oluştu (HATA 1)"); }
									elseif($verot_alert == 3) { alert_error("Yüklenirken bir hata oluştu (HATA 1)"); }
